# Ishibashi Web Clipper — Retired

> **Final maintenance release: 1.0.10**
>
> This original plugin is retired. Version 1.0.10 is a safety update for existing users: it makes no requests to clipped pages, no longer resolves redirects, and treats mobile share and clipboard input as URL-only. New users should install Ishibashi Web Clipper V2.
...
- Make no requests to clipped pages and fetch no remote metadata.
...
## Privacy behavior in 1.0.10

- No HTML, Open Graph, Twitter Card, image, description, or redirect requests.
- Mobile share and clipboard capture keep only the first URL and discard accompanying text.
- A confirmation screen is always shown for mobile share and clipboard capture.
- New notes record `content_source: user-provided` and `network_access: false`.
- The bookmarklet sends only the visible URL/title and text explicitly selected by the user.
...
1. Share a URL to Obsidian.
2. Choose `Save to Web Clips`.
3. Review the confirmation screen and create the Markdown note.
...
- Short or redirect URLs are stored as supplied and are not resolved automatically.
...
- Note template: Supports `{{date}}`, `{{title}}`, `{{url}}`, `{{note}}`, `{{site}}`, `{{domain}}`, and `{{tags}}`. Legacy `{{description}}` and `{{image}}` placeholders remain empty.
...
# Ishibashi Web Clipper 日本語ガイド（開発終了）

> **最終メンテナンス版：1.0.10**
>
> この旧プラグインの開発は終了しました。1.0.10は既存利用者向けの安全化更新です。保存先ページへの通信とリダイレクト解決を廃止し、スマホ共有・クリップボード入力をURL限定にします。新規利用はIshibashi Web Clipper V2を使用してください。
...
- HTML、OGP、Twitter Card、画像、description、リダイレクトを取得しない
- スマホ共有とクリップボードは最初のURLだけを読み、付随文を破棄する
- スマホ共有とクリップボードでは必ず確認画面を表示する
- 新規ノートに `content_source: user-provided` と `network_access: false` を記録する
- ブックマークレットは表示URL・タイトルと、利用者が明示的に選択した文字列だけを渡す
...
1. ブラウザやアプリからURLを共有します。
2. Obsidian側で `ウェブクリップに保存` を選びます。
3. 確認画面を確認し、Markdownノートを作成します。
...
- ノート本文テンプレート: `{{date}}`, `{{title}}`, `{{url}}`, `{{note}}`, `{{site}}`, `{{domain}}`, `{{tags}}` が使用可能。旧テンプレートの `{{description}}` と `{{image}}` は空になります。
